...
async function registerVisitor() {
    ...
    const photoData = document.querySelector('#preview img').src;
    ...
    try {
        ...
        const qrText = `Visitor: ${fullName}\nPlate: ${plateNumber}\nValid: ${visitDate} ${visitTime}`;
        document.getElementById('qr-code').innerHTML = "";
        QRCode.toCanvas(document.getElementById('qr-code'), qrText, function (error) {
            if (error) console.error(error);
        });

        document.getElementById('qr-details').innerText = qrText;
        document.getElementById('expiry-date').innerText = visitDate + " " + visitTime;
        document.getElementById('qr-result').classList.remove('hidden');
