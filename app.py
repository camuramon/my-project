...
def admin_dashboard():
    ...
    # ✅ Plate numbers mula sa visitors collection
    plate_numbers = [v.get("plateNumber", "N/A") for v in visitors if "plateNumber" in v]
    ...
    return render_template(
        ...
        total_plates=total_plates,       # ✅ bilang ng plate numbers
        plate_numbers=plate_numbers,     # ✅ listahan ng plates
...
@login_required(role="admin")
def admin_reports():
    # Get visitors data from Firestore
    visitors_ref = db.collection("visitors").stream()
    visitors = [dict(v.to_dict(), id=v.id) for v in visitors_ref]

    # Get reports data from Firestore
    reports_ref = db.collection("reports").stream()
    reports = [dict(r.to_dict(), id=r.id) for r in reports_ref]

    # Notifications (sample placeholder, adjust if you store these)
    notifications_ref = db.collection("notifications").stream()
    notifications = [n.to_dict() for n in notifications_ref]
