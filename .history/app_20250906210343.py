...
def admin_dashboard():
    ...
    visitors_ref = db.collection("visitors").stream()
    active_visitors = sum(1 for v in visitors_ref if v.to_dict().get("status") == "Active")

    # Count valid QR codes (same as active visitors)
    valid_qr_codes = active_visitors
    ...
    # Count entry logs today
    import datetime
    today = datetime.date.today()
    entry_logs_ref = db.collection("visitors").where("visitDate", "==", today.isoformat()).stream()
    entry_logs_today = sum(1 for _ in entry_logs_ref)
