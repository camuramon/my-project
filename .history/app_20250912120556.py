...
def admin_dashboard():
   ...
   for v in visitors_ref:
    ...
    # Kung meron expirationDate field
    if "expirationDate" in data and data["expirationDate"]:
        try:
            exp_date = datetime.strptime(data["expirationDate"], "%Y-%m-%d").date()
            if today <= exp_date:
                data["status"] = "Active"
        except Exception as e:
            print(f"⚠️ Error parsing expirationDate for visitor {data['id']}: {e}")

    # Fallback kung walang expirationDate → check visitDate
    elif "visitDate" in data and data["visitDate"]:
        try:
            visit_date = datetime.strptime(data["visitDate"], "%Y-%m-%d").date()
            if visit_date == today:
                data["status"] = "Active"
        except Exception as e:
            print(f"⚠️ Error parsing visitDate for visitor {data['id']}: {e}")
