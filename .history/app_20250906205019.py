...
def admin_dashboard():
    ...
    for v in visitors_ref:
        data = v.to_dict()
        data["id"] = v.id
        # Determine status based on visit date
        data["status"] = "Active" if "visitDate" in data and data["visitDate"] == str(today) else "Expired"
...
vehicles_list = [v.to_dict() for v in vehicles_ref]

# Only count vehicles linked to active residents or active visitors
active_vehicles = sum(
    1 for v in vehicles_list
    if v.get("ownerStatus") == "Active"  # assuming each vehicle has an ownerStatus field
)
    # Properties
