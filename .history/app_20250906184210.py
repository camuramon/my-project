...
def guard_dashboard():
    visitors_ref = db.collection("test_collection").order_by("created_at", direction=firestore.Query.DESCENDING).stream()
...
def add_visitor():
    ...
    print("✅ Visitor added:", name, purpose, doc_ref)
...
def admin_dashboard():
    visitors_ref = db.collection("test_collection").stream()
    visitors = [dict(v.to_dict(), id=v.id) for v in visitors_ref]
