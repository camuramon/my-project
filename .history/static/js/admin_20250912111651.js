...
const vehicleChart = new Chart(document.getElementById("vehicleChart"), {
  ...
  options: {
    responsive: false,   // ❌ huwag auto-resize
    maintainAspectRatio: false // ❌ payagan i-stretch depende sa container
  }
});
...
const residentsVisitorsChart = new Chart(document.getElementById("residentsVisitorsChart"), {
  ...
  options: {
    ...
    maintainAspectRatio: false // ❌ sundan yung container size
  }
});
// script.js
document.addEventListener("DOMContentLoaded", () => {
  // lahat ng 👁️ buttons
  const actionButtons = document.querySelectorAll("td .view-btn, td .eye-btn, td .action-eye");

  actionButtons.forEach(btn => {
    btn.addEventListener("click", () => {
      alert("Show details here...");
      // dito mo pwede ilagay yung modal popup or redirect
    });
  });
});
