...
# Reference to your root doc
root_doc = db.collection("test_collection").document("test_doc")
...
def guard_dashboard():
    visitors_ref = root_doc.collection("visitors").order_by("created_at", direction=firestore.Query.DESCENDING).stream()
...
def add_visitor():
    ...
    doc_ref = root_doc.collection("visitors").add({
        "name": name,
        "purpose": purpose,
        "status": "Active",
...
def admin_dashboard():
    visitors_ref = root_doc.collection("visitors").stream()
    visitors = [dict(v.to_dict(), id=v.id) for v in visitors_ref]
    ...
    entry_logs_today = sum(
        1 for v in visitors if v.get("created_at") and v["created_at"].date() == today
    )
