...
def add_visitor():
    ...
    # 1️⃣ Save uploaded photo 1
    photo1_file = request.files.get("photo1")
    if photo1_file and photo1_file.filename != "":
        photo1_filename = f"{name}_{plate}_1.png".replace(" ", "_")
        photo1_path = os.path.join(UPLOAD_FOLDER, photo1_filename)
        photo1_file.save(photo1_path)
        photo1_url = f"/{photo1_path}"
    else:
        photo1_url = "/static/images/default-visitor.png"

    # 2️⃣ Save uploaded photo 2
    photo2_file = request.files.get("photo2")
    if photo2_file and photo2_file.filename != "":
        photo2_filename = f"{name}_{plate}_2.png".replace(" ", "_")
        photo2_path = os.path.join(UPLOAD_FOLDER, photo2_filename)
        photo2_file.save(photo2_path)
        photo2_url = f"/{photo2_path}"
    else:
        photo2_url = "/static/images/default-visitor.png"

    # 3️⃣ Generate & save QR code
    ...
    # 4️⃣ Save visitor record to Firestore
    db.collection("visitors").add({
        ...
        "photoUrl1": photo1_url,   # ✅ visitor photo 1
        "photoUrl2": photo2_url,   # ✅ visitor photo 2
        "qrUrl": qr_url,           # ✅ QR code
        "status": "Active",
