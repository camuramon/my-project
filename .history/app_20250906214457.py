...
def add_visitor():
    ...
    # Add visitor
    visitor_doc = db.collection("visitors").add({
        "fullName": name,
        "plateNumber": plate,
        "photo": photo,
        "purpose": purpose,
    ...
    # Automatically create QR code for visitor
    db.collection("qr_codes").add({
        "visitor_id": visitor_doc[1].id,
        "status": "valid",
        "created_at": firestore.SERVER_TIMESTAMP
    })
...
def admin_dashboard():
    ...
    for v in visitors_ref:
        data = v.to_dict()
        data["id"] = v.id
        # Check if visitor has a valid QR
        qr_ref = db.collection("qr_codes").where("visitor_id", "==", v.id).where("status", "==", "valid").stream()
        has_valid_qr = any(qr_ref)
        # Active visitor if QR is valid and visitDate is today
        data["status"] = "Active" if has_valid_qr and "visitDate" in data and data["visitDate"] == str(today) else "Expired"
    ...
    # QR codes
    qr_codes_ref = db.collection("qr_codes").where("status", "==", "valid").stream()
