...
def generate_qr(data, name, plate):
    ...
    # ✅ Define relative path for web use
    qr_url = os.path.join(QR_FOLDER, qr_filename).replace("\\", "/")
...
def generate_qr(data, name, plate):
    ...
    # ✅ Relative path for Flask static serving
    qr_url = f"{QR_FOLDER}/{qr_filename}".replace("\\", "/")
...
def admin_dashboard():
    ...
    for v in visitors_ref:
        ...
        try:
            visit_date = datetime.strptime(data.get("visitDate", ""), "%Y-%m-%d").date()
            expiry_date = datetime.fromisoformat(data.get("expiryDate")).date()

            if visit_date <= today <= expiry_date:
                data["status"] = "Active"
                active_visitors += 1
                if visit_date == today:
                    entry_logs_today += 1
 ...
 # Vehicles
    vehicles_ref = db.collection("vehicles").stream()
    vehicles = [dict(v.to_dict(), id=v.id) for v in vehicles_ref]
    ...
    vehicles_this_month = [
        v for v in vehicles
        if "created_at" in v and v["created_at"] and v["created_at"].to_pydatetime() >= start_of_month
    ]
    vehicles_last_month = [
        v for v in vehicles
        if "created_at" in v and v["created_at"] and start_of_last_month <= v["created_at"].to_pydatetime() < start_of_month
    ]

    if len(vehicles_last_month) > 0:
        growth_percentage = round(((len(vehicles_this_month) - len(vehicles_last_month)) / len(vehicles_last_month)) * 100, 2)
    else:
        growth_percentage = 100 if len(vehicles_this_month) > 0 else 0

    return render_template(
        'admin/dashboard.html',
        ...
        total_vehicles=total_vehicles,
        vehicle_growth=growth_percentage,
        ...
    )
    return render_template(
        'admin/dashboard.html',
