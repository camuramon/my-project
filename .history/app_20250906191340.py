...
# -------------------------
# Debug Route (Check Firestore Data)
# -------------------------
@app.route("/debug_visitors")
def debug_visitors():
    try:
        visitors_ref = db.collection("visitors").stream()
        visitors = [dict(v.to_dict(), id=v.id) for v in visitors_ref]
        ...
        if not visitors:
            return "❌ No visitors found in Firestore."

        output = "<h2>Visitors in Firestore</h2><ul>"
        for v in visitors:
            output += f"<li>{v.get('name')} - {v.get('purpose')} - {v.get('created_at')}</li>"
        output += "</ul>"
        return output

    except Exception as e:
        return f"⚠️ Error fetching visitors: {e}"
