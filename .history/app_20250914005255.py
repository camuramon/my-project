...
def admin_qr():
    ...
    for v in visitors_ref:
        ...
        # 👇 Ensure these fields exist so Jinja won't error
        data["photoUrl"] = data.get("photo", "/static/img/default-photo.png")
        data["qrUrl"] = data.get("qrUrl", "/static/img/default-qr.png")
        data["status"] = data.get("status", "Active")

        visitors.append(data)

    # Recent visitors for notifications (top bar)
    notifications = visitors[-5:]
