    ...
    for v in visitors_ref:
        ...
        if "expirationDate" in data:
            try:
                ...
                data["status"] = "Active" if today <= exp_date else "Expired"
            except:
                data["status"] = "Expired"
        elif "visitDate" in data:
            data["status"] = "Active" if data["visitDate"] == str(today) else "Expired"
        else:
            data["status"] = "Expired"
...
def add_visitor():
    ...
    expiration_date = request.form.get("expiration-date")  # optional field
    ...
    if expiration_date:
        visitor_data["expirationDate"] = expiration_date
