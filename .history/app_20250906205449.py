...
def admin_dashboard():
    ...
    total_residents = sum(1 for _ in residents_ref)

    # --- Active Visitors ---
    visitors_ref = db.collection("visitors").stream()
    visitors_list = [v.to_dict() for v in visitors_ref]
    active_visitors = sum(1 for v in visitors_list if v.get("status") == "Active")

    # --- Valid QR Codes for Active Visitors ---
    # assuming each visitor document has a 'qrCodeId' field
    valid_qr_codes = sum(
        1 for v in visitors_list
        if v.get("status") == "Active" and v.get("qrCodeId")
    )

    # --- Vehicles Registered ---
    vehicles_ref = db.collection("vehicles").stream()
    total_vehicles = sum(1 for _ in vehicles_ref)
    ...
    entry_logs_today = sum(
        1 for v in visitors_list
        if v.get("visitDate") == today.strftime("%Y-%m-%d")
    )

    # --- Properties ---
    properties_ref = db.collection("properties").stream()
    total_properties = sum(1 for _ in properties_ref)
